...
public class GuardImplBug extends CharacterImpl implements Guard {
    ...
    public Move getBehaviour() {
        ...
        if(cell == Cell.LAD) {
            if((cell_below == Cell.PLT || cell_below == Cell.MTL || guard_below) && (hDist > vDist || vDist == 0)) { // Oups
    ...
    public void climbRight() {
        ...
        Cell nat_arrivee = getEnvi().getCellNature(getCol()-1, getHgt()+1); // Oups
        if(nat_arrivee == Cell.PLT || nat_arrivee == Cell.MTL) return;
        for(InCell ic: getEnvi().getCellContent(getCol()+1, getHgt()+1)) {
            if(ic instanceof Guard) return;
        }
    ...
    @Override
    public Guard clone() {
        GuardImplBug g = new GuardImplBug();
        g.id = getId();
