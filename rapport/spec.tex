...
{\small
  \begin{longtable}{rl}
    ...
    \textbf{Observations}:&\\
    \texttt{[TriggerTrap]}: & \texttt{CellNature(TriggerTrap(S,x,y),x,y)} $=$ \textbf{EMP} \\
    & $\forall$ $(x,y)$ $\in$ \texttt{[0;Width(S)[}$\times$ \texttt{[0;Height(S)[}, \\
    & \quad\quad (\texttt{x} $\neq$ \texttt{u} $\lor$ \texttt{y} $\neq$ \texttt{v}) $\Rightarrow$ \texttt{CellNature(TriggerTrap(S,u,v)),x,y)} $=$ \texttt{CellNature(x,y)} \\
  \end{longtable}
}
